<template>
  <BlocksResponsiveInfo v-model="isOpen" sidePanelClass="w-120">
    <div class="m-4 flex flex-col gap-3">
      <h2 class="text-3xl font-headers tracking-tight">Variations</h2>
      <div v-if="variations.length === 0" class="text-sm text-gray-400">
        No other variations found.
      </div>
      <div v-else class="flex flex-col gap-3">
        <div v-for="(v, index) in variations" :key="index">
          <RecipeCardHorizontal :recipe="v" :uniqueId="`variation-${v.id}-${index}`" />
        </div>
      </div>
    </div>
