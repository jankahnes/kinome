  ...
  <div class="flex flex-col gap-4 mt-8 lg:mt-2">
    <div class="flex flex-col w-full gap-2">
      <div class="flex items-end xl:justify-center">
        <div>
          <input type="text" v-model="form.targetInputs.value.kcal.input"
            class="text-8xl focus:outline-none bg-transparent border-none font-semibold h-18"
      ...
      <div class="w-full h-2 rounded-full overflow-hidden flex">
        <div class="h-full bg-carbs" :style="{ width: `${macroBarWidth('carbohydrates', 4)}%` }"></div>
        <div class="h-full bg-protein" :style="{ width: `${macroBarWidth('protein', 4)}%` }"></div>
        <div class="h-full bg-fat" :style="{ width: `${macroBarWidth('fat', 9)}%` }"></div>
      </div>
    </div>
    ...
    <div class="flex gap-4 justify-between mt-6 flex-col xl:flex-row">
      <div class="flex flex-col flex-1">
        <div class="flex items-center justify-between">
          <label for="carbohydrates" class="text-2xl font-headers leading-none">Carbohydrates</label>
          <IconLock class="text-gray-600 w-5" v-if="form.targetInputs.value.carbohydrates.locked" strokeWidth="2.3"
            @click="form.targetInputs.value.carbohydrates.locked = false" />
          <IconUnlock class="text-gray-400 w-5" v-else strokeWidth="2.3"
            @click="form.targetInputs.value.carbohydrates.locked = true" />
        </div>
        <div class="flex mt-6 items-end justify-between">
          <input id="carbohydrates" type="text" v-model="form.targetInputs.value.carbohydrates.input"
            class="text-6xl focus:outline-none bg-transparent border-none flex-1 font-semibold w-full h-14" />
      ...
      <div class="flex flex-col flex-1">
        <div class="flex items-center justify-between">
          <label for="protein" class="text-2xl font-headers leading-none">Protein</label>
          <IconLock class="text-gray-600 w-5" v-if="form.targetInputs.value.protein.locked" strokeWidth="2.3"
            @click="form.targetInputs.value.protein.locked = false" />
          <IconUnlock class="text-gray-400 w-5" v-else strokeWidth="2.3"
            @click="form.targetInputs.value.protein.locked = true" />
        </div>
        <div class="flex mt-6 items-end justify-between">
          <input id="protein" type="text" v-model="form.targetInputs.value.protein.input"
            class="text-6xl focus:outline-none bg-transparent border-none flex-1 font-semibold w-full h-14" />
          <div class="flex gap-2 text-2xl text-gray-500 items-center select-none">
            <span class="cursor-pointer"
              :class="{ 'font-semibold text-gray-700': form.targetInputs.value.protein.selectedUnit === 'g' }"
              @click="form.toggleProteinUnit()">g</span>
            <span>|</span>
            <span class="cursor-pointer"
              :class="{ 'font-semibold text-gray-700': form.targetInputs.value.protein.selectedUnit === '%' }"
              @click="form.toggleProteinUnit()">%</span>
          </div>
        </div>
      ...
      <div class="flex flex-col flex-1">
        <div class="flex items-center justify-between">
          <label for="fat" class="text-2xl font-headers leading-none">Fat</label>
          <IconLock class="text-gray-600 w-5" v-if="form.targetInputs.value.fat.locked" strokeWidth="2.3"
            @click="form.targetInputs.value.fat.locked = false" />
          <IconUnlock class="text-gray-400 w-5" v-else strokeWidth="2.3"
            @click="form.targetInputs.value.fat.locked = true" />
        </div>
        <div class="flex mt-6 items-end justify-between">
          <input id="fat" type="text" v-model="form.targetInputs.value.fat.input"
            class="text-6xl focus:outline-none bg-transparent border-none flex-1 font-semibold w-full h-14" />
          <div class="flex gap-2 text-2xl text-gray-500 items-center select-none">
            <span class="cursor-pointer"
              :class="{ 'font-semibold text-gray-700': form.targetInputs.value.fat.selectedUnit === 'g' }"
              @click="form.toggleFatUnit()">g</span>
            <span>|</span>
            <span class="cursor-pointer"
              :class="{ 'font-semibold text-gray-700': form.targetInputs.value.fat.selectedUnit === '%' }"
              @click="form.toggleFatUnit()">%</span>
          </div>
        </div>
    ...
    <h3 class="text-2xl font-headers">Additional Targets</h3>
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
      <div class="flex xl:justify-center flex-1">
        <div class="flex flex-col gap-4 shrink-0 flex-1">
          <label for="sugar" class="text-2xl font-headers leading-none">Sugar</label>
          <div class="flex gap-2 items-end border-b-2 border-gray-300 pb-2">
            <input id="sugar" type="text" v-model="form.targetInputs.value.sugar.input"
          ...
          <div class="flex gap-2">
            <button class="bg-primary-5 main-card-rounded px-3 py-1 text-xs font-mono self-start"
      ...
      <div class="flex xl:justify-center flex-1">
        <div class="flex flex-col gap-4 shrink-0 flex-1">
          <label for="fiber" class="text-2xl font-headers leading-none">Fiber</label>
          <div class="flex gap-2 items-end border-b-2 border-gray-300 pb-2">
            <input id="fiber" type="text" v-model="form.targetInputs.value.fiber.input"
              class="text-4xl focus:outline-none bg-transparent font-bold flex-1 w-[2.1ch] h-8" />
            <span class="text-2xl text-gray-500 leading-none">g</span>
          </div>
          <div class="flex gap-2">
      ...
      <div class="flex xl:justify-center flex-1">
        <div class="flex flex-col gap-4 shrink-0 flex-1">
          <label for="salt" class="text-2xl font-headers leading-none">Salt</label>
          <div class="flex gap-2 items-end border-b-2 border-gray-300 pb-2">
            <input id="salt" type="text" v-model="form.targetInputs.value.salt.input"
              class="text-4xl focus:outline-none bg-transparent font-bold flex-1 w-[2.1ch] h-8" />
            <span class="text-2xl text-gray-500 leading-none">g</span>
          </div>
          <div class="flex gap-2">
