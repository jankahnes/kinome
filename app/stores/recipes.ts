...
export const useRecipeStore = defineStore('recipe', () => {
  ...
  const INSTAGRAM_DELAY = 1000; // 2 seconds between Instagram requests
  ...
  async function getSocialPicture(
    ...
    if (/instagram\.com/.test(source)) {
      ...
      const waitTime = Math.max(0, INSTAGRAM_DELAY - timeSinceLastFetch);

      // Reserve this slot by updating lastInstagramFetch before waiting
      lastInstagramFetch = now + waitTime;
