...
export default function getNutritionHighlightCards(
  ...
  for (const category of nutritionHighlightsCategories) {
    ...
    if (category.name === 'Protein') {
      ...
      const details = computable?.report?.micronutrients?.details ?? [];
      const top3 = [...details]
      ...
      const fatProfile = computable?.report?.humanReadable?.fatProfile;
      if (fatProfile && Array.isArray(fatProfile)) {
        const highest = [...fatProfile].sort(
          (a, b) => (b.value ?? 0) - (a.value ?? 0)
        )[0];
        if (highest?.description) {
          const desc = highest.description.toLowerCase();
          // Parse the description to create contextual subtitle - TEMPORARY FIX
          if (desc.includes('omega-3')) {
            subtitle = 'High in Omega-3s';
          } else if (desc.includes('saturated')) {
            subtitle = 'Low in Saturated Fat';
          } else if (desc.includes('monounsaturated')) {
            subtitle = 'High in Healthy Fats';
          } else if (desc.includes('trans')) {
            subtitle = 'No Trans Fat';
          } else {
            subtitle = highest.description;
          }
        }
      }
    } else if (category.name === 'Protective Compounds') {
      const protective = computable?.report?.humanReadable?.protectiveCompounds;
      if (protective && Array.isArray(protective)) {
        const highest = [...protective].sort(
          (a, b) => (b.value ?? 0) - (a.value ?? 0)
        )[0];
        if (highest?.description) {
          const desc = highest.description.toLowerCase();
          // Extract the compound name from description - TEMPORARY FIX
          if (desc.includes('polyphenol')) {
            subtitle = 'Rich in Polyphenols';
          } else if (desc.includes('carotenoid')) {
            subtitle = 'Rich in Carotenoids';
          } else if (desc.includes('glucosinolate')) {
            subtitle = 'Rich in Glucosinolates';
          } else {
            subtitle = highest.description;
          }
        }
