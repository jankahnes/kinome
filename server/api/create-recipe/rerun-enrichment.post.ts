...
export default defineEventHandler(async (event) => {
  ...
  if (recipe.visibility === 'PUBLIC') {
    throw createError({ statusCode: 409, statusMessage: 'Recipe is already public' });
  }

  // Set PUBLISH_PENDING + fresh publish_start so the 5-min stuck timer resets
  await supabase
    .from('recipes')
  ...
  $fetch('/api/create-recipe/postprocess-enrich-recipe', {
    ...
    console.error(`[rerun-enrichment] Phase B failed for recipe ${recipeId}:`, err);
  });
